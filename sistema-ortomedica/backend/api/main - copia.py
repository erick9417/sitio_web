...
def ingest_run():
    ...
    ingest_state.started_at = dt.datetime.utcnow().isoformat()

    cmd = os.getenv("INGEST_CMD", "python run_scrapers.py")
    threading.Thread(target=_run_ingest_cmd, args=(cmd,), daemon=True).start()
...
def ingest_run():
    ...
    def _worker():
        try:
            # Lanza tu scraper. Si está en otro .py cámbialo aquí.
            # También podrías llamar una función interna si la tienes como módulo.
...
def _run_ingest_cmd(cmd: str):
    try:
        ...
        if rc == 0:
            ...
            ingest_state.last_success_at = now_iso    # <-- solo éxito
            ingest_state.last_error_at = None
            ingest_state.last_error_msg = None
        else:
            ingest_state.status = "error"
            ingest_state.last_error_at = now_iso
            ingest_state.last_error_msg = f"Proceso terminó con código {rc}"
        # Nota: NO tocar started_at; sirve para auditoría del último start
    except Exception as e:
        ingest_state.busy = False
        ingest_state.status = "error"
        ingest_state.last_error_at = dt.datetime.utcnow().isoformat()
        ingest_state.last_error_msg = str(e)
...
def list_products(
    ...
    params: Dict[str, Any] = {}
    if q and q.strip():
        where = "WHERE (i.sku LIKE %(q_like)s OR i.name LIKE %(q_like)s)"
