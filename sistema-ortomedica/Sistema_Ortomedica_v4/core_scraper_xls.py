...
def process_spreadsheet(data: bytes, suggested_name: str, system_code: str, store_id: int, colmap: Dict[str, str]) -> int:
    ...
    for i, r in enumerate(rows, start=1):
        ...
        # Guarda crudo (incluye duplicados)
        insert_inventory_raw(run_id, system_code, store_id, system_id, sku, name, existencia, precio, costo, i)

        # Guarda normalizado, clave por source_id
        if system_id:
            pid = ensure_product_and_alias(system_code, system_id, sku, name, store_id)
            upsert_stock(pid, store_id, system_code, existencia, precio, costo)
            total += 1
